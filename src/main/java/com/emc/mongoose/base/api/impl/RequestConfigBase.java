 ...
 */
public class RequestConfigBase<T extends DataItem>
implements RequestConfig<T> {
	...
	@Override
	public RequestConfigBase<T> setAddr(final String addr) {
		this.addr = addr;
		return this;
	}
	//
	...
	@Override
	public RequestConfigBase<T> setPort(final int port)
	...
	//
	@Override
	public RequestConfigBase<T> setProperties(final RunTimeConfig props) {
	...
	public RequestConfigBase<T> clone() {
		return new RequestConfigBase<T>()
			.setAPI(getAPI())
			.setAddr(getAddr())
