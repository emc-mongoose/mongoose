...
public final class Main {
  ...
  private static Config applyArgsToConfig(
    ...
    try {
      ...
      final String formattedAllCliArgs =
          allCliArgs(config.schema(), config.pathSep())
              .stream()
  ...
  private static void runNode(
      final Config config, final List<Extension> extensions, final MetricsManager metricsMgr)
