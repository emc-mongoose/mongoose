 ...
 */
public final class WSPoolProducer<T extends WSObject>
extends Thread
implements Producer<T> {
	...
	private final WSPoolImpl<T> pool;
	private final Constructor<T> dataConstructor;
	private final long maxCount;
	private final String addr;
	//
	@SuppressWarnings("unchecked")
	public WSPoolProducer(
			final WSPoolImpl<T> pool, final Class<? extends WSObject> dataCls, final long maxCount,
			final String addr
...
*/
		...
		try {
			httpResp = pool.execute(addr, MutableWSRequest.HTTPMethod.GET);

		} catch(final IOException e) {
			LogUtil.exception(LOG, Level.ERROR, e, "Failed to list the pool: " + pool);
		}
		//
		//
		if(httpResp != null) {
			...
			if(statusLine == null) {
				...
				if(statusCode >= 200 && statusCode < 300) {
						...
						if(ContentType.APPLICATION_XML.getMimeType().equals(respContentType)) {
							try {
								...
								try(final InputStream in = respEntity.getContent()) {
									...
									LogUtil.exception(
										LOG, Level.ERROR, e,
										"Failed to read the pool listing response content: {}",
											pool
									);
								} finally {
									if(consumer != null) {
					...
					LOG.warn(
						Markers.ERR, "Listing pool \"{}\" response: {}/{}",
							pool, statusCode, statusMsg
					);
				}
			}
		}
