...
public class PrometheusMetricsExporterImpl extends Collector implements PrometheusMetricsExporter {
      ...
      mfsList.add(
          new MetricFamilySamples(
              ...
              Collections.singletonList(
                  new Sample(
                      String.format(METRIC_FORMAT, METRIC_NAME_TIME, "value"),
                      labelNames,
                      labelValues,
  ...
  private List<Sample> collect(final RateMetricSnapshot metric) {
    ...
    samples.add(
        new Sample(
            String.format(METRIC_FORMAT, metricName, "count"),
            labelNames,
            labelValues,
            metric.count()));
    samples.add(
        new Sample(
            String.format(METRIC_FORMAT, metricName, "rate_mean"),
            labelNames,
            labelValues,
            metric.mean()));
    samples.add(
        new Sample(
            String.format(METRIC_FORMAT, metricName, "rate_last"),
            labelNames,
            labelValues,
            metric.last()));
    return samples;
  ...
  private List<Sample> collect(final TimingMetricSnapshot metric) {
    ...
    samples.add(
        new Sample(
            String.format(METRIC_FORMAT, metricName, "sum"),
            labelNames,
            labelValues,
            metric.sum() / Constants.M));
    samples.add(
        new Sample(
            String.format(METRIC_FORMAT, metricName, "mean"),
            labelNames,
            labelValues,
            metric.mean() / Constants.M));
    samples.add(
        new Sample(
            String.format(METRIC_FORMAT, metricName, "min"),
            labelNames,
            labelValues,
    ...
    for (int i = 0; i < quantileValues.size(); ++i) {
      final Sample sample =
          new Sample(
              String.format(METRIC_FORMAT, metricName, "quantile_" + quantileValues.get(i)),
              labelNames,
              labelValues,
    ...
    samples.add(
        new Sample(
            String.format(METRIC_FORMAT, metricName, "max"),
            labelNames,
            labelValues,
  ...
  private List<Sample> collect(final ConcurrencyMetricSnapshot metric) {
    ...
    samples.add(
        new Sample(
            String.format(METRIC_FORMAT, metricName, "last"),
            labelNames,
            labelValues,
            metric.last()));
    return samples;
