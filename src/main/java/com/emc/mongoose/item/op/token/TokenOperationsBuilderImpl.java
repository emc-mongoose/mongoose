...
implements TokenOperationsBuilder<I, O> {
	...
	throws IOException {
		final String uid;
		return (O) new TokenOperationImpl<>(
			originIndex, opType, item, Credential.getInstance(uid = getNextUid(), getNextSecret(uid))
		);
	}
	...
	throws IOException {
		...
		for(final I item : items) {
			buff.add(
				(O) new TokenOperationImpl<>(
					originIndex, opType, item,
