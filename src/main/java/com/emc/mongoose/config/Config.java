...
implements Serializable {
	...
	@JsonProperty(KEY_NODE) private boolean nodeModeFlag;
	@JsonProperty(KEY_OUTPUT) private OutputConfig outputConfig;
	@JsonProperty(KEY_SCENARIO) private ScenarioConfig scenarioConfig;
	...
	public Config(final Config config) {
		...
		this.nodeModeFlag = config.getNode();
	...
	public final void setNode(final boolean nodeModeFlag) {
		this.nodeModeFlag = nodeModeFlag;
	}
	 ...
	 */
	public void apply(final Map<String, Object> tree, final String autoGeneratedIdOffer)
	throws IllegalArgumentException {

		final StepConfig stepConfig = getScenarioConfig().getStepConfig();
		final String oldStepId = stepConfig.getId();
		final boolean tmpIdFlag = stepConfig.getIdTmp();
		...
		String newStepId = stepConfig.getId();
		// if was no test id before or test id was temporary (auto generated)
		if(newStepId == null || (newStepId.equals(oldStepId) && tmpIdFlag)) {
			newStepId = autoGeneratedIdOffer;
			stepConfig.setId(newStepId);
	...
	public static Config loadFromUrl(final URL configUrl)
	throws IOException {
		final ObjectMapper mapper = getObjectMapper();
