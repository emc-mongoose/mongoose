...
implements ObjectLoadClient<T> {
	...
	//
	private final Map<String, LoadService<T>> remoteLoadMap;
	////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//
	public WSLoadClient(
		final Map<String, LoadService<T>> remoteLoadMap,
	...
	private void logMetaInfoFrames() {
		...
		//
		for(final LoadService<T> nextLoadSvc: remoteLoadMap.values()) {
			try {
	...
	implements Runnable {
		//
		private final LoadExecutor<T> driver;
		private final T dataItem;
		//
		private SubmitTask(final LoadExecutor<T> driver, final T dataItem) {
			this.driver = driver;
	...
	public final void setConsumer(final Consumer<T> load)
		...
		try { // consumer is map of consumers
			final WSLoadClient<T> loadClient = (WSLoadClient<T>) load;
			final Map<String, LoadService<T>> consumeMap = loadClient.remoteLoadMap;
