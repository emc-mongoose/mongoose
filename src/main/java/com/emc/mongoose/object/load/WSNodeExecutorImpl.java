 ...
 */
public class WSNodeExecutorImpl<T extends WSObject>
extends ThreadPoolExecutor
implements WSNodeExecutor<T> {
	//
	private final static Logger LOG = LogManager.getLogger();
	//
	private final WSRequestConfig<T> localReqConf;
	...
	//
	private WSNodeExecutorImpl(
		final int threadsPerNode, final WSRequestConfig<T> localReqConf,
	...
	public final void submit(final T dataItem) {
		WSRequest<T> request = null;
		try {
			request = (WSRequest<T>) WSRequestImpl
	...
	protected final void afterExecute(final Runnable reqTask, final Throwable thrown) {
		if(thrown!=null) {
			...
			try(final WSRequest<T> request = (WSRequest<T>) Future.class.cast(reqTask).get()) {
