...
implements ObjectLoadExecutor<T> {
	...
	protected ObjectLoadExecutorBase(
		...
		nodes = new ObjectNodeExecutor[nodeCount];
	...
	protected abstract void initClient(
		final int totalThreadCount, final ObjectRequestConfig<T> reqConf
	) throws ClassCastException;
	protected abstract void setFileBasedProducer(final String listFile);
	protected abstract void initNodeExecutors(
		final String addrs[], final ObjectRequestConfig<T> reqConf
	) throws ClassCastException;
	//
	@Override
	public void start() {
	...
	public final synchronized void interrupt() {
		...
		for(int i = 0; i < nodes.length; i ++) {
			final ObjectNodeExecutor<T> nextNode = nodes[i];
	...
	public synchronized void close()
		...
		for(final ObjectNodeExecutor<T> nodeExecutor: nodes) {
			try {
				nodeExecutor.close();
	...
	private void logMetrics(final Marker logMarker) {
		...
		if(LOG.isDebugEnabled(Markers.PERF_AVG)) {
			for(final ObjectNodeExecutor<T> node: nodes) {
