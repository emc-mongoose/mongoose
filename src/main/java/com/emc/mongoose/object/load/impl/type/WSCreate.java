 ...
 */
public class WSCreate<T extends WSDataObjectBase>
extends WSLoadExecutor<T> {
	...
	public WSCreate(
		final String[] addrs, final WSObjectRequestConfig<T> recConf, final long maxCount,
		...
		if(producer==null) { // otherwise producer is reader specified by "listFile"
			producer = new WSBasicDataItemProducer(minObjSize, maxObjSize);
			producer.setConsumer(this);
		}
	}
	//
	final class WSBasicDataItemProducer
	extends Thread
	implements Producer<T> {
		...
		//
		public WSBasicDataItemProducer(final long minObjSize, final long maxObjSize) {
