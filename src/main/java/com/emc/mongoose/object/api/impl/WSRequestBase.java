...
import com.emc.mongoose.object.api.impl.provider.WSAtmos;
import com.emc.mongoose.object.load.impl.type.WSRead;
 ...
 */
public class WSRequestBase<T extends WSDataObject>
implements WSObjectRequest<T> {
	//
	private final static Logger LOG = LogManager.getLogger();
	public final static WSObjectRequest POISON = new WSRequestBase();
	//
	private WSObjectRequestConfig<T> reqConf = null;
	...
	private final static ConcurrentHashMap<WSObjectRequestConfig, GenericInstancePool<WSObjectRequest>>
		POOL_MAP = new ConcurrentHashMap<>();
	//
	@SuppressWarnings("unchecked")
	public static WSObjectRequest getInstanceFor(
		...
		if(dataItem==null) {
			...
			synchronized(POOL_MAP) {
				if(POOL_MAP.containsKey(reqConf)) {
					...
					pool = new GenericInstancePool<>(WSRequestBase.class);
	...
	public final void close() {
		final GenericInstancePool<WSObjectRequest> pool = POOL_MAP.get(reqConf);
		pool.release(this);
	}
	...
	public WSObjectRequest<T> setRequestConfig(final RequestConfig<T> reqConf) {
		if(this.reqConf==null) { // request instance has not been configured yet?
			...
			switch(reqConf.getLoadType()) {
				case CREATE:
					httpRequest = WSAtmos.class.getSimpleName().equals(reqConf.getAPI()) ?
	...
	//
	@Override
	public final WSObjectRequest<T> setDataItem(final T dataItem) {
		try {
	...
	//
	@Override
	public final WSObjectRequest<T> call()
	...
	//
	@Override
	public final WSObjectRequest<T> handleResponse(final HttpResponse httpResponse) {
		//
			...
			if(statusCode < 300) {
				switch(httpRequest.getMethod()) {
					...
					case (HttpGet.METHOD_NAME):
						if(WSRead.VERIFY_CONTENT) { // validate the response content
