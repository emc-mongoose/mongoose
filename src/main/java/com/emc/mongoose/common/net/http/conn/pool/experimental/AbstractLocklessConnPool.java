 ...
 */
@ThreadSafe
public abstract class AbstractLocklessConnPool<T, C, E extends PoolEntry<T, C>>
implements ConnPool<T, E>, ConnPoolControl<T> {
	...
	private final ConcurrentLinkedQueue<LeaseRequest<T, C, E>> leasingRequests;
	private final Set<SessionRequest> pending;
	private final Set<E> leased;
	private final ConcurrentLinkedQueue<E> available;
	 ...
	 * @since 4.3
	 */
	public AbstractLocklessConnPool(
		...
		this.leased = new ConcurrentSkipListSet<>();
		...
		if (pool == null) {
			pool = new RouteSpecificPool<T, C, E>(route) {
				...
				protected E createEntry(final T route, final C conn) {
					return AbstractLocklessConnPool.this.createEntry(route, conn);
				}
	...
	private boolean processPendingRequest(final LeaseRequest<T, C, E> request) {
		...
		if (pool.getAllocatedCount() < maxPerRoute) {
			...
			if (totalAvailable > freeCapacity - 1) {
				final E lastUsed = this.available.poll();
				if(lastUsed != null) {
					lastUsed.close();
