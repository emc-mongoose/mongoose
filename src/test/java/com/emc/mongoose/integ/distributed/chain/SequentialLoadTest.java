...
public class SequentialLoadTest {
	//
	private final static String LOAD_SEQ = "create,read,delete";
	//
	private static Logger LOG;
	private static long LOAD_LIMIT_TIME_SEC = 20, DURATION_TOTAL_SEC = -1;
	//
	@BeforeClass
	...
	throws Exception {
		...
		Assert.assertTrue(
			...
			DURATION_TOTAL_SEC >= countSteps * LOAD_LIMIT_TIME_SEC
		);
		Assert.assertTrue(
			"Actual duration (" + DURATION_TOTAL_SEC + "[s]) is too much more than expected (" +
			countSteps * LOAD_LIMIT_TIME_SEC + "[s])",
			DURATION_TOTAL_SEC - 100 < countSteps * LOAD_LIMIT_TIME_SEC
		);
	}
}
