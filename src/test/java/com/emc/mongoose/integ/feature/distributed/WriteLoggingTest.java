		...
		try(
			...
			do {
				...
				if(nextStdOutLine == null) {
					...
					if(m.find()) {
						Assert.assertTrue(
							"Load type is not " + AppConfig.LoadType.WRITE.name(),
							AppConfig.LoadType.WRITE.name().toLowerCase().equals(
			...
			for(final CSVRecord nextRec : recIter) {
				if(firstRow) {
					...
					Assert.assertTrue(
						...
						AppConfig.LoadType.WRITE.name().equalsIgnoreCase(nextRec.get(3))
					);
				}
			}
		}
