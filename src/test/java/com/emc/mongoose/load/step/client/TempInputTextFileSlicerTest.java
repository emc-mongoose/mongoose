...
public final class TempInputTextFileSlicerTest {
	...
	static {
		try {
			SRC_FILE_NAME = Files
				.createTempFile(TempInputTextFileSlicerTest.class.getSimpleName(), ".txt")
				.toString();
	...
	private static final List<FileManager> FILE_MGRS = IntStream
		.range(0, 10)
		.mapToObj(i -> new FileManagerImpl())
		.collect(Collectors.toList());
	private static final Map<FileManager, String> FILE_SLICES = FILE_MGRS
		.stream()
		.collect(
			Collectors.toMap(
				Function.identity(),
