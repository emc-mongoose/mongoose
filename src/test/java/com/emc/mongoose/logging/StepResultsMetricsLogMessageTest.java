...
public class StepResultsMetricsLogMessageTest extends StepResultsMetricsLogMessage {
  ...
  static {
    ...
    final TimingMetricSnapshot dS =
        new TimingMeterImpl(h, MetricsConstants.METRIC_NAME_DUR).snapshot();
    h = new HistogramImpl(new ConcurrentSlidingWindowLongReservoir(COUNT));
    LongStream.of(LATENCIES).forEach(h::update);
    final TimingMetricSnapshot lS =
        new TimingMeterImpl(h, MetricsConstants.METRIC_NAME_LAT).snapshot();
    h = new HistogramImpl(new ConcurrentSlidingWindowLongReservoir(COUNT));
    LongStream.of(CONCURRENCIES).forEach(h::update);
    final ConcurrencyMetricSnapshot cS =
        new ConcurrencyMeterImpl(MetricsConstants.METRIC_NAME_CONC).snapshot();
    final RateMetricSnapshot fS =
        new RateMetricSnapshotImpl(0, 0, MetricsConstants.METRIC_NAME_FAIL, 0, 0);
    final RateMetricSnapshot sS =
        new RateMetricSnapshotImpl(
            COUNT / durSum, COUNT / durSum, MetricsConstants.METRIC_NAME_SUCC, COUNT, 0);
    final RateMetricSnapshot bS =
        new RateMetricSnapshotImpl(
            COUNT / durSum,
            COUNT / durSum,
  ...
  public final void testIsValidYaml() throws Exception {
    ...
    final Map<String, Object> parsed =
        ((Map<String, Object>[]) mapper.readValue(buff.toString(), parsedType))[0];
