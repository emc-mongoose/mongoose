...
import com.emc.mongoose.util.LogValidationUtil;
...
public class TlsReadUsingInputFileTest {
	...
	private static final String ITEM_LIST_FILE = CONTAINER_SHARE_PATH + "/"
		+ snakeCaseName(TlsReadUsingInputFileTest.class) + ".csv";
		...
		switch(storageType) {
			...
			case SWIFT:
				final HttpStorageMockContainer storageMock = new HttpStorageMockContainer(
					HttpStorageMockContainer.DEFAULT_PORT, true, null, null,
	...
	throws Exception {
		...
		assertEquals(
			Integer.toString(msgCount), 2 * runMode.getNodeCount() * concurrency.getValue(), msgCount, msgCount / 10.0
		);
		...
		testTotalMetricsLogRecord(
			...
			itemSize.getValue(), OBJ_COUNT_LIMIT, 0, TIME_LIMIT_SECONDS
		);
		...
		testMetricsTableStdout(
			stdOutContent.replaceAll("[\r\n]+", " "), stepId, storageType, runMode.getNodeCount(), OBJ_COUNT_LIMIT,
			concurrencyMap
		);
	}
}
