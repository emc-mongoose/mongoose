...
@RunWith(Parameterized.class) public class SingleRandomUpdateAndMultipleRandomReadTest {
	...
	private final String hostItemOutputPath =
		MongooseEntryNodeContainer.getHostItemOutputPath(getClass().getSimpleName());
	...
	throws Exception {
		...
		testContainer = new MongooseEntryNodeContainer(stepId, storageType, runMode, concurrency, itemSize.getValue(),
			SCENARIO_PATH, env, args
		);
	}
	...
	throws Exception {
		...
		testTotalMetricsLogRecord(totalMetrcisLogRecordsUpdate.get(0), OpType.UPDATE, concurrency.getValue(),
			runMode.getNodeCount(), expectedUpdateSize, EXPECTED_COUNT, 0, 0
		);
		...
		testTotalMetricsLogRecord(totalMetrcisLogRecordsRead.get(0), OpType.READ, concurrency.getValue(),
			runMode.getNodeCount(), expectedReadSize, EXPECTED_COUNT, 0, 0
		);
		...
		testFinalMetricsStdout(stdOutContent, OpType.UPDATE, concurrency.getValue(), runMode.getNodeCount(),
			expectedUpdateSize, stepIdUpdate
		);
		testFinalMetricsStdout(stdOutContent, OpType.READ, concurrency.getValue(), runMode.getNodeCount(),
			expectedReadSize, stepIdRead
		);
	}
}
