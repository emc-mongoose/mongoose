...
public class UnlimitedConcurrencyLimitByRateTest {
    ...
    public final void test()
        ...
        testSingleMetricsStdout(
                stdOutContent, CREATE, 0, runMode.getNodeCount(), itemSize.getValue(),
                averagePeriod
        );

        final List<CSVRecord> metricsLogRecs = getMetricsLogRecords(stepId);
        testMetricsLogRecords(
                metricsLogRecs, CREATE, 0, runMode.getNodeCount(), itemSize.getValue(), COUNT_LIMIT,
                TIME_LIMIT_SEC,
                averagePeriod
        );
        ...
        testTotalMetricsLogRecord(
                totalMetricsRec, CREATE, 0, runMode.getNodeCount(), itemSize.getValue(), COUNT_LIMIT,
                TIME_LIMIT_SEC
        );
