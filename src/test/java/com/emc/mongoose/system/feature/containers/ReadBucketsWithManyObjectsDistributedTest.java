...
extends DistributedLoadBuilderTestBase {
	...
	public static void setUpClass()
		...
		System.setProperty(AppConfig.KEY_DATA_SIZE, "1");
		...
		rtConfig.set(AppConfig.KEY_CREATE_CONNS, "10");
		...
		rtConfig.set(AppConfig.KEY_RUN_ID, RUN_ID_BASE + "Write");
		rtConfig.set(AppConfig.KEY_ITEM_CLASS, "data");
		rtConfig.set(AppConfig.KEY_LOAD_LIMIT_COUNT, LIMIT_COUNT_OBJ);
		RunTimeConfig.setContext(rtConfig);
		try(
			...
			do {
				...
				if(nextContainer == null) {
					...
					rtConfig.set(AppConfig.KEY_API_S3_BUCKET, nextContainer);
					RunTimeConfig.setContext(rtConfig);
					new ScenarioRunner().run();
					TimeUnit.SECONDS.sleep(1);
				}
		...
		//
		rtConfig.set(AppConfig.KEY_RUN_ID, RUN_ID_BASE + "Read");
		rtConfig.set(AppConfig.KEY_ITEM_CLASS, "container");
		rtConfig.set(AppConfig.KEY_ITEM_SRC_FILE, containerListFile.toString());
		rtConfig.set(AppConfig.KEY_SCENARIO_SINGLE_LOAD, "read");
		rtConfig.set(AppConfig.KEY_READ_CONNS, "10");
		rtConfig.set(AppConfig.KEY_LOAD_LIMIT_COUNT, 0);
		RunTimeConfig.setContext(rtConfig);
		//
		new ScenarioRunner().run();
		TimeUnit.SECONDS.sleep(1);
