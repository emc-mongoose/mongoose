		...
		try(
			...
			for(final CSVRecord nextRec : recIter) {
				if(firstRow) {
					...
					Assert.assertEquals("DurationMax[us]", nextRec.get(19));
					Assert.assertEquals("LatencyAvg[us]", nextRec.get(20));
					Assert.assertEquals("LatencyMin[us]", nextRec.get(21));
					Assert.assertEquals("LatencyMax[us]", nextRec.get(22));
					firstRow = false;
				} else {
					secondRow = true;
