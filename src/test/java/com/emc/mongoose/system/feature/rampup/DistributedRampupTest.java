...
extends DistributedLoadBuilderTestBase {
	...
	throws Exception {
		System.setProperty(AppConfig.KEY_RUN_ID, DistributedRampupTest.class.getCanonicalName());
		DistributedLoadBuilderTestBase.setUpClass();
		//
		final RunTimeConfig rtConfig = RunTimeConfig.getContext();
		rtConfig.set(AppConfig.KEY_SCENARIO_CHAIN_CONCURRENT, "false");
		rtConfig.set(AppConfig.KEY_LOAD_LIMIT_TIME, Long.toString(LOAD_LIMIT_TIME_SEC) + "s");
		rtConfig.set(AppConfig.KEY_LOAD_METRICS_PERIOD_SEC, "0");
		rtConfig.set(AppConfig.KEY_RUN_MODE, Constants.RUN_MODE_CLIENT);
		rtConfig.set(AppConfig.KEY_API_NAME, "swift");
		rtConfig.set(AppConfig.KEY_SCENARIO_CHAIN_LOAD, LOAD_SEQ);
		rtConfig.set(AppConfig.KEY_SCENARIO_RAMPUP_SIZES, SIZE_SEQ);
		rtConfig.set(AppConfig.KEY_SCENARIO_RAMPUP_CONN_COUNTS, THREAD_COUNT_SEQ);
