...
extends DistributedClientTestBase {
	...
	throws Exception {
		System.setProperty(AppConfig.KEY_RUN_ID, DeleteLoggingTest.class.getCanonicalName());
		...
		try(
			...
			COUNT_WRITTEN = client.create(itemIO, COUNT_LIMIT, 10, SizeInBytes.toFixedSize("10KB"));
		...
		try(
			...
			do {
				...
				if(nextStdOutLine == null) {
					...
					if(m.find()) {
						Assert.assertTrue(
							"Load type is not " + LoadType.DELETE.name() + ": " + m.group("typeLoad"),
							LoadType.DELETE.name().equalsIgnoreCase(m.group("typeLoad"))
						);
						long
