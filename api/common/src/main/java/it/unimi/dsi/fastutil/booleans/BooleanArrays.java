...
public class BooleanArrays {
	...
	protected static class ForkJoinQuickSort extends RecursiveAction {
		...
		protected void compute() {
			...
			while (true) {
				int comparison;
				while (b <= c
						&& (comparison = (Boolean.compare((x[b]), (v)))) <= 0) {
					if (comparison == 0)
						swap(x, a++, b);
					b++;
				}
				while (c >= b
						&& (comparison = (Boolean.compare((x[c]), (v)))) >= 0) {
					if (comparison == 0)
						swap(x, c, d--);
					c--;
				}
	...
	public static void quickSortIndirect(final int[] perm, final boolean[] x,
		...
		while (true) {
			int comparison;
			while (b <= c
					&& (comparison = (Boolean.compare((x[perm[b]]), (v)))) <= 0) {
				if (comparison == 0)
					IntArrays.swap(perm, a++, b);
				b++;
			}
			while (c >= b
					&& (comparison = (Boolean.compare((x[perm[c]]), (v)))) >= 0) {
				if (comparison == 0)
	...
	public static void quickSort(final boolean[] x, final boolean[] y,
		...
		while (true) {
			int comparison, t;
			while (b <= c
					&& (comparison = (t = (Boolean.compare((x[b]), (v)))) == 0
			...
			while (c >= b
					&& (comparison = (t = (Boolean.compare((x[c]), (v)))) == 0
	...
	public final static Hash.Strategy<boolean[]> HASH_STRATEGY = new ArrayHashStrategy();
