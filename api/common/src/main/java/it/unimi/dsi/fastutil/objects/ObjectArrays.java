 ...
 * <em>significantly slower</em> than using <code>new</code>. This phenomenon is
...
public class ObjectArrays {
	 ...
	 * the same class as of those of <code>prototype</code>. In case of an empty
	 ...
	 * you should probably use <code>grow()</code> instead.
	 *
	 * @param array
	 *            an array.
	 * @param length
	 ...
	 *         first <code>array.length</code> entries are the same as those of
	 *         <code>array</code>.
	 */
	 ...
	 *         or more; otherwise, an array with <code>length</code> entries
	 *         whose first <code>preserve</code> entries are the same as those
	 *         of <code>array</code>.
	 */
	public static <K> K[] ensureCapacity(final K[] array, final int length,
			final int preserve) {
	 ...
	 *         <code>array.length</code> entries are the same as those of
	 *         <code>array</code>.
	 * */
	 ...
	 *         entries; otherwise, an array with max(<code>length</code>,
	 *         <code>array.length</code>/&phi;) entries whose first
	 *         <code>preserve</code> entries are the same as those of
	 *         <code>array</code>.
	 * */
	 ...
	 * @return <code>array</code>, if it contains <code>length</code> entries or
	 *         less; otherwise, an array with <code>length</code> entries whose
	 *         entries are the same as the first <code>length</code> entries of
	 *         <code>array</code>.
	 * 
	 */
	 ...
	 *         <code>length</code> entries, an array with <code>length</code>
	 *         entries whose entries are the same as the first
	 *         <code>length</code> entries of <code>array</code>; otherwise, an
	 *         array with <code>length</code> entries whose first
	 *         <code>array.length</code> entries are the same as those of
	 *         <code>array</code>.
	 * 
	 */
	 ...
	 * @return a new array containing <code>length</code> elements of
	 *         <code>array</code> starting at <code>offset</code>.
	 */

	public static <K> K[] copy(final K[] array, final int offset,
			final int length) {
	...
	@Deprecated
	public static <K> void fill(final K[] array, final int from, int to,
			final K value) {
	 ...
	 *             if <code>from</code> or <code>to</code> are greater than the
	 *             array length or negative.
	 */
	public static <K> void ensureFromTo(final K[] a, final int from,
			final int to) {
	 ...
	 */
	public static <K> void ensureOffsetLength(final K[] a, final int offset,
			final int length) {
	...
	private static <K> int med3(final K x[], final int a, final int b,
			final int c, Comparator<K> comp) {
	...
	private static <K> void selectionSort(final K[] a, final int from,
			final int to, final Comparator<K> comp) {
	...
	private static <K> void insertionSort(final K[] a, final int from,
			final int to, final Comparator<K> comp) {
		for (int i = from; ++i < to;) {
			K t = a[i];
			int j = i;
	...
	protected static class ForkJoinQuickSortComp<K> extends RecursiveAction {
		...
		protected void compute() {
			...
			if (s > 1 && t > 1)
				invokeAll(
						new ForkJoinQuickSortComp<K>(x, from, from + s, comp),
						new ForkJoinQuickSortComp<K>(x, to - t, to, comp));
			else if (s > 1)
	 ...
	 *            the comparator to determine the sorting order.
	 */
	public static <K> void parallelQuickSort(final K[] x, final int from,
			final int to, final Comparator<K> comp) {
	...
	public static <K> void quickSort(final K[] x, final int from, final int to) {
		...
		while (true) {
			int comparison;
			while (b <= c
					&& (comparison = (((Comparable<K>) (x[b])).compareTo(v))) <= 0) {
				if (comparison == 0)
					swap(x, a++, b);
				b++;
			}
			while (c >= b
					&& (comparison = (((Comparable<K>) (x[c])).compareTo(v))) >= 0) {
				if (comparison == 0)
					swap(x, c, d--);
				c--;
			}
		...
		protected void compute() {
			...
			if (s > 1 && t > 1)
				invokeAll(new ForkJoinQuickSort<K>(x, from, from + s),
						new ForkJoinQuickSort<K>(x, to - t, to));
			else if (s > 1)
				invokeAll(new ForkJoinQuickSort<K>(x, from, from + s));
			else
	...
	@SuppressWarnings("unchecked")
	private static <K> int med3Indirect(final int perm[], final K x[],
	...
	@SuppressWarnings("unchecked")
	private static <K> void insertionSortIndirect(final int[] perm,
	 ...
	 * 
	 * <p>
	 * This method implement an <em>indirect</em> sort. The elements of
	 * <code>perm</code> (which must be exactly the numbers in the interval
	 * <code>[0..perm.length)</code>) will be permuted so that
	...
	public static <K> void quickSortIndirect(final int[] perm, final K[] x,
		...
		while (true) {
			int comparison;
			while (b <= c
					&& (comparison = (((Comparable<K>) (x[perm[b]]))
							.compareTo(v))) <= 0) {
				if (comparison == 0)
			...
			while (c >= b
					&& (comparison = (((Comparable<K>) (x[perm[c]]))
							.compareTo(v))) >= 0) {
				if (comparison == 0)
	...
	protected static class ForkJoinQuickSortIndirect<K> extends RecursiveAction {
		...
		protected void compute() {
			...
			t = d - c;
			if (s > 1 && t > 1)
				invokeAll(new ForkJoinQuickSortIndirect<K>(perm, x, from, from
						+ s), new ForkJoinQuickSortIndirect<K>(perm, x, to - t,
						to));
			else if (s > 1)
				invokeAll(new ForkJoinQuickSortIndirect<K>(perm, x, from, from
						+ s));
	 ...
	 *            the index of the last element (exclusive) to be sorted.
	 */
	public static <K> void parallelQuickSortIndirect(final int[] perm,
	 ...
	 * <code>x[ perm[ i ] ] &le; x[ perm[ i + 1 ] ]</code>, after stabilization
	 * we will also have that <code>x[ perm[ i ] ] = x[ perm[ i + 1 ] ]</code>
	 * implies <code>perm[ i ] &le; perm[ i + 1 ]</code>.
	 *
	 * @param perm
	...
	@SuppressWarnings("unchecked")
	private static <K> int med3(final K x[], final K[] y, final int a,
	...
	private static <K> void swap(final K x[], final K[] y, final int a,
			final int b) {
		final K t = x[a];
		final K u = y[a];
		x[a] = x[b];
	...
	private static <K> void swap(final K[] x, final K[] y, int a, int b,
			final int n) {
		for (int i = 0; i < n; i++, a++, b++)
			swap(x, y, a, b);
	}
	...
	@SuppressWarnings("unchecked")
	private static <K> void selectionSort(final K[] a, final K[] b,
			final int from, final int to) {
		for (int i = from; i < to - 1; i++) {
			int m = i, u;
			for (int j = i + 1; j < to; j++)
				if ((u = (((Comparable<K>) (a[j])).compareTo(a[m]))) < 0
						|| u == 0
						&& (((Comparable<K>) (b[j])).compareTo(b[m]) < 0))
					m = j;
	 ...
	 * 
	 * <p>
	 * This method implements a <em>lexicographical</em> sorting of the
	 ...
	 * end, either <code>x[ i ] &lt; x[ i + 1 ]</code> or <code>x[ i ]
	...
	@SuppressWarnings("unchecked")
	public static <K> void quickSort(final K[] x, final K[] y, final int from,
			final int to) {
		final int len = to - from;
		if (len < QUICKSORT_NO_REC) {
		...
		while (true) {
			int comparison, t;
			while (b <= c
					&& (comparison = (t = (((Comparable<K>) (x[b]))
							.compareTo(v))) == 0 ? (((Comparable<K>) (y[b]))
							.compareTo(w)) : t) <= 0) {
				if (comparison == 0)
					swap(x, y, a++, b);
				b++;
			...
			while (c >= b
					&& (comparison = (t = (((Comparable<K>) (x[c]))
							.compareTo(v))) == 0 ? (((Comparable<K>) (y[c]))
							.compareTo(w)) : t) >= 0) {
				if (comparison == 0)
					swap(x, y, c, d--);
				c--;
		...
		protected void compute() {
			...
			if (s > 1 && t > 1)
				invokeAll(new ForkJoinQuickSort2<K>(x, y, from, from + s),
						new ForkJoinQuickSort2<K>(x, y, to - t, to));
			else if (s > 1)
				invokeAll(new ForkJoinQuickSort2<K>(x, y, from, from + s));
			else
	 ...
	 *            the index of the last element (exclusive) to be sorted.
	 * @param supp
	 *            a support array containing at least <code>to</code> elements,
	...
	public static <K> void mergeSort(final K a[], final int from, final int to,
		...
		for (int i = from, p = from, q = mid; i < to; i++) {
			if (q >= to || p < mid
					&& (((Comparable<K>) (supp[p])).compareTo(supp[q]) <= 0))
				a[i] = supp[p++];
			else
				a[i] = supp[q++];
		}
	 ...
	 * <p>
	 * This sort is guaranteed to be <i>stable</i>: equal elements will not be
	 * reordered as a result of the sort. An array as large as <code>a</code>
	 ...
	 */
	public static <K> void mergeSort(final K a[], final int from, final int to,
			Comparator<K> comp, final K supp[]) {
		int len = to - from;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <K> int binarySearch(final K[] a, int from, int to,
			final K key) {
		K midVal;
		to--;
		while (from <= to) {
	 ...
	 */
	public static <K> int binarySearch(final K[] a, int from, int to,
			final K key, final Comparator<K> c) {
		K midVal;
		to--;
		while (from <= to) {
			final int mid = (from + to) >>> 1;
	 ...
	 * @param random
	 *            a pseudorandom number generator (please use a <a href=
	 *            "http://dsiutils.dsi.unimi.it/docs/it/unimi/dsi/util/XorShiftStarRandom.html"
	 *            >XorShift*</a> generator).
	 * @return <code>a</code>.
	 */
	public static <K> K[] shuffle(final K[] a, final int from, final int to,
			final Random random) {
