 ...
 * {@link ObjectArrays} apply here. Moreover, you
...
public class ObjectArrayList<K> extends AbstractObjectList<K>
	 ...
	 * Otherwise, the returned array will be of type {@link Object Object[]} (in
	...
	private void grow(final int capacity) {
		...
		if (wrapped)
			...
			if (capacity > a.length) {
				final int newLength = (int) Math.max(Math.min(2L * a.length,
	...
	public void getElements(final int from, final Object[] a, final int offset,
			final int length) {
	 ...
	 */
	public void addElements(final int index, final K a[], final int offset,
			final int length) {
		ensureIndex(index);
		ObjectArrays.ensureOffsetLength(a, offset, length);
		grow(size + length);
