...
public class Main {
	...
	throws IOException, InterruptedException, UserShootHisFootException {
		...
		for(int i = 0; i < generatorCount; i ++) {
			...
			drivers.add(
				(Driver<I, O>) new BasicFileDriver<>(
					config.getLoadConfig(), config.getIoConfig().getBufferConfig()
				)
			);
			drivers.add(
				(Driver<I, O>) new BasicFileDriver<>(
			...
			generators.add(
				new BasicGenerator<>(
					drivers, LoadType.CREATE, (ItemFactory<I>) new BasicMutableDataItemFactory(),
