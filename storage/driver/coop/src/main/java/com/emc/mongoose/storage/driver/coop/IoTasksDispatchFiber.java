...
public final class IoTasksDispatchFiber<I extends Item, O extends IoTask<I>>
	...
	public IoTasksDispatchFiber(
		...
		final BlockingQueue<O> inTasksQueue, final BlockingQueue<O> childTasksQueue, final String stepId,
		final int batchSize
	) {
		this(
			executor, new OptLockArrayBuffer<>(batchSize), storageDriver, inTasksQueue, childTasksQueue, stepId,
			batchSize
		);
	}
	...
	protected final void invokeTimedExclusively(final long startTimeNanos) {
		...
		try {
			...
			LogUtil.exception(
				Level.DEBUG, e, "{}: failed to submit some I/O tasks due to the illegal storage driver state ({})",
