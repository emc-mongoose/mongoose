...
public class SwiftStorageDriver<I extends Item, O extends Operation<I>>
  ...
  private static final ThreadLocal<StringBuilder> CONTAINER_LIST_QUERY =
  ...
  protected final String requestNewPath(final String path) throws InterruptRunException {
    ...
    final FullHttpRequest checkContainerReq =
    ...
    if (!containerExists || (!versioningEnabled && versioning)) {
      ...
      final FullHttpRequest putContainerReq =
          new DefaultFullHttpRequest(
              HttpVersion.HTTP_1_1,
              HttpMethod.PUT,
              containerUri,
  ...
  protected final String requestNewAuthToken(final Credential credential)
    ...
    final FullHttpRequest getAuthTokenReq =
        new DefaultFullHttpRequest(
            HttpVersion.HTTP_1_1,
            HttpMethod.GET,
            AUTH_URI,
  ...
  private HttpRequest manifestCreateRequest(
    ...
    final String objManifestPath =
        super.dataUriPath(item, srcPath, compositeDataOp.dstPath(), CREATE);
    httpHeaders.set(
  ...
  private HttpRequest uploadPartRequest(
    ...
    final String uriPath =
        dataUriPath(item, srcPath, partialDataOp.dstPath(), CREATE)
            + "/"
