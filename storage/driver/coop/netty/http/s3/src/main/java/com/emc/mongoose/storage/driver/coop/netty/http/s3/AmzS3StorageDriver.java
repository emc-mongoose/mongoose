...
public class AmzS3StorageDriver<I extends Item, O extends Operation<I>>
  ...
  private static final ThreadLocal<Map<String, Mac>> MAC_BY_SECRET =
  ...
  protected String requestNewPath(final String path) throws InterruptRunException {
    ...
    if (!bucketExistedBefore) {
      ...
      final FullHttpRequest putBucketReq =
          new DefaultFullHttpRequest(
              HttpVersion.HTTP_1_1,
              HttpMethod.PUT,
              bucketUri,
    ...
    final FullHttpRequest getBucketVersioningReq =
        new DefaultFullHttpRequest(
            HttpVersion.HTTP_1_1,
            HttpMethod.GET,
  ...
  protected void enableBucketVersioning(
    ...
    final var putBucketVersioningReq =
        (FullHttpRequest)
            new DefaultFullHttpRequest(
                HttpVersion.HTTP_1_1,
                HttpMethod.PUT,
  ...
  public final List<I> list(
    ...
    final var countLimit =
        count < 1 || count > AmzS3Api.MAX_KEYS_LIMIT ? AmzS3Api.MAX_KEYS_LIMIT : count;
    final var nodeAddr = storageNodeAddrs[0];
  ...
  private HttpRequest getInitMpuRequest(final O op, final String nodeAddr) {
    ...
    final var httpRequest =
        (HttpRequest) new DefaultHttpRequest(HTTP_1_1, httpMethod, uriPath, httpHeaders);
  ...
  private HttpRequest getUploadPartRequest(
    ...
    final var uriPath =
        dataUriPath(item, srcPath, partialDataOp.dstPath(), OpType.CREATE)
            + "?partNumber="
  ...
  private static final ThreadLocal<StringBuilder> THREAD_LOCAL_STRB =
  ...
  private FullHttpRequest getCompleteMpuRequest(
    ...
    final var uriPath =
        dataUriPath(item, srcPath, mpuTask.dstPath(), OpType.CREATE) + "?uploadId=" + uploadId;
    final HttpHeaders httpHeaders = new DefaultHttpHeaders();
    httpHeaders.set(HttpHeaderNames.HOST, nodeAddr);
    final var httpMethod = HttpMethod.POST;
