  ...
  static <I extends Item> int parseContainerListing(
    ...
    try (final JsonParser jsonParser = new JsonFactory().createParser(inStream)) {
      ...
      if (JsonToken.START_ARRAY.equals(rootToken)) {
            ...
            case END_OBJECT:
              if (isInsideObjectToken) {
                if (lastItemId != null && lastSize > -1) {
                  ...
                  try {
                    nextItem = itemFactory.getItem(path_ + lastItemId, lastItemOffset, lastSize);
                    if (nextItem != null) {
                      buff.add(nextItem);
                      n++;
                    }
                  } catch (final IllegalStateException e) {
                    LogUtil.exception(Level.WARN, e, "Failed to create data item descriptor");
                  }
                } else {
                  Loggers.ERR.trace("Invalid object id ({}) or size ({})", lastItemId, lastSize);
                }
              } else {
        ...
        Loggers.ERR.warn("Response contains root JSON token \"{}\", but array token was expected");
      }
    }
