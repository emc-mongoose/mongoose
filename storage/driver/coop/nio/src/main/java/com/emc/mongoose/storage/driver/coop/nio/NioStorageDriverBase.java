...
/** Created by kurila on 19.07.16. The multi-threaded non-blocking I/O storage driver. */
public abstract class NioStorageDriverBase<I extends Item, O extends Operation<I>>
    extends CoopStorageDriverBase<I, O> implements NioStorageDriver<I, O> {
  ...
  /**
   * The class represents the non-blocking load operation execution algorithm. The load operation
   * itself may correspond to a large data transfer so it can't be non-blocking. So the load
   * operation may be invoked multiple times (in the reentrant manner).
   */
  private final class NioWorkerTask extends ExclusiveFiberBase {
    ...
    public NioWorkerTask(
        final FibersExecutor executor, final CircularBuffer<O> opBuff, final Lock opBuffLock) {
  ...
  /**
   * Reentrant method which decorates the actual non-blocking create/read/etc I/O operation. May
   * change the task status or not change if the I/O operation is not completed during this
  ...
  protected final void finishOperation(final O op) {
    try {
      ...
      LogUtil.exception(
          Level.WARN,
          e,
          "{}: finishing the load operation which is in an invalid state",
          op.toString());
  ...
  @Override
  protected void doClose() throws IOException, InterruptRunException {
    ...
    for (int i = 0; i < ioWorkerCount; i++) {
      try (final Instance logCtx = CloseableThreadContext.put(KEY_CLASS_NAME, CLS_NAME)) {
        if (opBuffLocks[i].tryLock(Fiber.WARN_DURATION_LIMIT_NANOS, TimeUnit.NANOSECONDS)) {
          ...
          Loggers.ERR.debug(
              new ThreadDumpMessage("Failed to obtain the load operations buff lock in time"));
        }
