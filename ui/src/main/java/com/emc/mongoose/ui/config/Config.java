...
implements Serializable {
	...
	public Config(final Config config) {
		...
		final var ac = config.getAliasingConfig();
	...
	/**
	 ...
	 autogenerated value. Then if the test-step-id value was not set and applied
	...
	throws IllegalArgumentException {
		...
		final var testStepConfg = getTestConfig().getStepConfig();
		final var oldTestStepId = testStepConfg.getId();
		final var tmpTestIdFlag = testStepConfg.getIdTmp();
		...
		var newTestStepId = testStepConfg.getId();
	...
	private static void applyAliasing(
		...
		for(final var nextAliasNode : rawAliases) {
			...
			for(var i = 0; i < aliasNamePath.length; i ++) {
	...
	private static void cleanEmptyPaths(final Map<String, Object> tree) {
		...
		var emptyBranchFound = true; // assume
		Object t;
	...
	throws InvocationTargetException, IllegalAccessException {
		...
		for(final var key : branch.keySet()) {
			final var node = branch.get(key);
			if(node instanceof Map) {
				final var childBranch = (Map<String, Object>) node;
				try {
					final var subConfigGetter = configCls.getMethod(
						"get" + capitalize(key) + "Config"
					);
					final var subConfig = subConfigGetter.invoke(config);
					try {
	...
	throws InvocationTargetException, IllegalAccessException {
		final Class configCls = config.getClass();
		try {
			final var fieldGetter = configCls.getMethod("get" + capitalize(key));
			...
			if(value == null) {
				...
				if(TypeUtil.typeEquals(fieldType, valueType)) {
					...
					if(fieldType.equals(List.class)) {
						final var listValue = Arrays.asList(((String) value).split(","));
						configCls
							.getMethod("set" + capitalize(key), List.class)
							.invoke(config, listValue);
					} else if(fieldType.equals(Map.class)) {
						final var field = (Map<String, String>) fieldGetter.invoke(config);
						...
						final var intValue = Integer.parseInt((String) value);
						configCls
						...
						try {
							final var longValue = Long.parseLong((String) value);
							configCls
								.getMethod("set" + capitalize(key), Long.TYPE)
								.invoke(config, longValue);
						} catch(final NumberFormatException e) {
							final var timeValue = TimeUtil.getTimeInSeconds((String) value);
							configCls
								.getMethod("set" + capitalize(key), Long.TYPE)
								.invoke(config, timeValue);
						}
					} else if(fieldType.equals(Float.TYPE) || fieldType.equals(Float.class)) {
						final var floatValue = Float.parseFloat((String) value);
						configCls
						...
						final var doubleValue = Double.parseDouble((String) value);
						configCls
	...
	public final Config replace(
		...
		final var mapper = new ObjectMapper()
			...
			.configure(JsonParser.Feature.ALLOW_YAML_COMMENTS, true);
		try {
			final var configText = mapper.writeValueAsString(this);
			final String newConfigText;
			final var rp = Pattern.quote(replacePattern);
			if(newValue == null) {
				...
				final var newValues = (List<Object>) newValue;
				final var newStrValues = new ArrayList<String>();
				...
				final var newValueStr = "[" + String.join(",", newStrValues) + "]";
			...
			if(v instanceof String) {
				...
				if(valueStr.equals("\"" + replacePattern + "\"")) {
					...
					if(newValue == null) {
						...
						for(final var newValueElement : (List) newValue) {
	...
	public final String toString() {
		final var mapper = new ObjectMapper().configure(SerializationFeature.INDENT_OUTPUT, true);
