...
public interface LogPatterns {
  ...
  Pattern STD_OUT_METRICS_TABLE_ROW =
      Pattern.compile(
          "\\s*(?<stepName>[\\w\\-_.,;:~=+@]{1,10})\\|(?<timestamp>[\\d]{12})"
              ...
              + "\\|\\s*(?<lat>[\\d]{1,10})"
              + "\\|\\s*(?<dur>[\\d]{1,11})");
  Pattern STD_OUT_METRICS_SUMMARY =
      Pattern.compile(
          "\\s*[\\-]{3}\\s#\\sResults\\s[#]{110}\n(?<content>[\\w\\s\\.:\\-\\[\\]/\n]+)[\\.]{3}",
          Pattern.MULTILINE);

  Pattern STD_OUT_LOAD_THRESHOLD_ENTRANCE =
      Pattern.compile(
          ASCII_COLOR.pattern()
  ...
  Pattern STD_OUT_LOAD_THRESHOLD_EXIT =
      Pattern.compile(
          ASCII_COLOR.pattern()
              + DATE_TIME_ISO8601.pattern()
              + "\\s+"
