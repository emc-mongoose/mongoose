...
/**
 * This class is used ONLY for input pattern strings containing only one expression with the pattern
 * symbol.
 */
public class PatternDefinedSupplierImpl implements PatternDefinedSupplier {

	private static final ThreadLocal<StringBuilder> OUTPUT_BUILDER = ThreadLocal.withInitial(StringBuilder::new);
	...
	// An input string with pattern symbols and expressions that have to be replaced by suitable
	// values
	private final String pattern;

	/**
	 * Generators of values that should be inserted instead of expressions with special characters
	 * (see above)
	 */
	private BatchSupplier<String>[] suppliers;

	public PatternDefinedSupplierImpl(final String pattern) throws OmgShootMyFootException {
		this(pattern, StringSupplierFactory.getInstance());
	}

	public PatternDefinedSupplierImpl(
	...
	@SuppressWarnings("unchecked") // AsyncStringGeneratorFactory always returns ValueGenerator<String> values for
	// getSuppliers[]
	protected void initialize() throws OmgShootMyFootException {
		...
		final String seedStr = initParameter(patternBuilder, RangeDefinedSupplier.SEED_BRACKETS);
		setSuppliers(
						new BatchSupplier[]{supplierFactory.createSupplier(type, seedStr, formatStr, null)
						});
	}
	...
	/**
	 * @param result - a parameter to create an opportunity of StringBuilder reusing (StringBuilder
	 *     instance must be cleared with setLength(0))
	 * @return a string with PATTERN_SYMBOLs replaced by suitable values
	 */
	@Override
