...
public final class RangePatternDefinedSupplier extends PatternDefinedSupplierImpl {

  /** Segments (parts) of the input string that do not require changes */
  private String[] segments;

  public RangePatternDefinedSupplier(final String pattern) throws OmgShootMyFootException {
    super(pattern);
  }
  ...
  private static final ThreadLocal<StringBuilder>
      THREAD_SB_0 = ThreadLocal.withInitial(StringBuilder::new),
      THREAD_SB_1 = ThreadLocal.withInitial(StringBuilder::new);
    ...
    if (patternSymbolsNum > 0) {
      ...
      for (int j = 0; j < patternSymbolsNum; j++) {
        ...
        segments[segmentCounter] =
            segmentsBuilder.toString(); // adding of the segment in 'segments' filed
        segmentsBuilder.setLength(0);
  ...
  /**
   * In this method is used to fill the 'inputs' field with value inputs in accordance with the
   * specified expression parameters
   *
  ...
  private void addExpressionParams(final StringBuilder expression, final int index)
    ...
    final String seed = initParameter(expression, RangeDefinedSupplier.SEED_BRACKETS);
    final String format = initParameter(expression, FORMAT_BRACKETS);
    final String range = initParameter(expression, RangeDefinedSupplier.RANGE_BRACKETS);
