...
public interface ConfigUtil {
	...
	static ObjectMapper readConfigMapper(final Map<String, Object> schema)
		...
		final JsonDeserializer<BasicConfig> deserializer = new ConfigJsonDeserializer<>(BasicConfig.class, "-", schema);
	...
	static Config merge(final String pathSep, final List<Config> configs) {
		final Map<String, Object> schema = configs.stream()
						.map(Config::schema)
						.reduce(TreeUtil::addBranches)
						.orElseGet(Collections::emptyMap);
		final Map<String, Object> configTree = configs.stream()
