...
extends ScenarioTestBase {
	...
	throws Exception {
		...
		EXCLUDE_PARAMS.put(KEY_ENV_STORAGE_DRIVER_concurrency.getValue(), Arrays.asList(1000));
		EXCLUDE_PARAMS.put(
			KEY_ENV_itemSize.getValue(),
			Arrays.asList(new SizeInBytes(0), new SizeInBytes("100MB"), new SizeInBytes("10GB"))
		);
		stepId = SingleRandomUpdateAndMultipleRandomReadTest.class.getSimpleName();
		...
		EXPECTED_UPDATE_SIZE = new SizeInBytes(1, itemSize.getValue().get(), 1);
